...
void sortArrays(SortPred pred, int len, uchar *chars, STATE_PTR_TYPE *ptrs, short *ptrTypes, int *matches, PatternTable* patternTable, STATE_PTR_TYPE state) {
	...
	int numPatterns;
	char **patterns = patternTable->patterns[state];
	int** patternsLength = patternTable->patternsLengths[state];

	HoffmanTable** hoffmanTable = patternTable->additionalCodes[state];
	int** hasAdditionalCodes = patternTable->hasAdditionalCodes[state];
	char** hoffmanCode = patternTable->hoffmanCode[state];
	int** hoffmanCodeLen = patternTable->hoffmanCodeLen[state];

	HoffmanTable* tmpHoffmanTable[MAX_ARRAY_LENGTH];
	int* tmpHasAdditionalCodes[MAX_ARRAY_LENGTH];
	char* tmpHoffmanCode[MAX_ARRAY_LENGTH];
	int* tmpHoffmanCodeLen[MAX_ARRAY_LENGTH];
	...
	if (numPatterns > 1) {
		...
		for (j = 0; j < len && i < numPatterns; j++) {
			if (indexes[j] >= 0) {
				...
				tmpHoffmanTable[i] = hoffmanTable[indexes[j]];
				tmpHasAdditionalCodes[i] = hasAdditionalCodes[indexes[j]];
				tmpHoffmanCode[i] = hoffmanCode[indexes[j]];
				tmpHoffmanCodeLen[i] = hoffmanCodeLen[indexes[j]];
				i++;
			}
		}

		memcpy(patterns, tmpPatterns, sizeof(char*) * numPatterns);
		memcpy(hoffmanTable, tmpHoffmanTable, sizeof(HoffmanTable*) * numPatterns);
		memcpy(hasAdditionalCodes, tmpHasAdditionalCodes, sizeof(int*) * numPatterns);
		memcpy(hoffmanCode, tmpHoffmanCode, sizeof(char*) * numPatterns);
		memcpy(hoffmanCodeLen, tmpHoffmanCodeLen, sizeof(int*) * numPatterns);
...
void setDataBranchState(StateMachine *machine, State *state, uchar *chars, STATE_PTR_TYPE *gotos, short *ptrTypes, int *matches, STATE_PTR_TYPE failure) {
	...
	case STATE_TYPE_BITMAP_ENCODED:
		sortArrays(&sortPredAlphabetically, ((StateHeader*)state)->size, chars, gotos, ptrTypes, matches, machine->patternTable, getStateID_LT(state));
		return setStateData_BM(state, chars, gotos, ptrTypes, matches, failure);
	case STATE_TYPE_LOOKUP_TABLE:
		GET_NUM_GOTOS_LT(state, res);
		sortArrays(&sortPredAlphabetically, res, chars, gotos, ptrTypes, matches, machine->patternTable, getStateID_LT(state));
...
void printState(State* state, int depth) {
	...
	case STATE_TYPE_BITMAP_ENCODED:
		...
		break;
	case STATE_TYPE_LOOKUP_TABLE:
		printState_LT(state, depth);
		break;
	case STATE_TYPE_PATH_COMPRESSED:
		printState_PC(state, depth);
		break;
	}
}

int getAllNextStates(State *state, int *next) {
	switch (GET_STATE_TYPE(state)) {
	case STATE_TYPE_LOOKUP_TABLE:
		return getAllNextStates_LT(state, next);
		break;
	case STATE_TYPE_PATH_COMPRESSED:
		return getAllNextStates_PC(state, next);
		break;
	}
	return -1;
}

...
#define MAX_FAIL_CODE_LENGTH 64
...
typedef struct {
	...
	short ptrType;
	unsigned char* hoffmanCode[MAX_FAIL_PATH_LENGTH];
	int hoffmanCodeLen[MAX_FAIL_PATH_LENGTH];
	} NextStatePtr;
	...
	if (nextNode->id < 0)
	{
		while (nextNode->id < 0)
		{
			if (nextNode->match)
			{
				...
				next->match = 1;
				if (nextNode->numOfPatterns == 0)
				{
					next->messages[next->numMessages] = nextNode->message;
					next->messagesLen[next->numMessages] = nextNode->messageLength;
					next->hoffmanCode[next->numMessages] = nextNode->huffman;
					next->hoffmanCodeLen[next->numMessages++] = nextNode->huffman_len;
				}
				else
				{
					int k;
					for(k=0;k<nextNode->numOfPatterns;k++)
					{
						next->messages[next->numMessages] = nextNode->patterns[k];
						next->messagesLen[next->numMessages] = strlen(nextNode->patterns[k]);
						next->hoffmanCode[next->numMessages] = nextNode->hoffmanCodes[k];
						next->hoffmanCodeLen[next->numMessages++] = nextNode->hoffmanCodesLength[k];
					}
				}
			}
		...
		if (node->numGotos > 1) {
			...
			for (i = 0; i < node->numGotos; i++) {
				for (k = 0; k < nexts[i].numMessages; k++) {
					...
					patterntable_add_pattern(patternTable, node->id, j, nexts[i].messages[k], nexts[i].messagesLen[k], nexts[i].hoffmanCode[k], nexts[i].hoffmanCodeLen[k]);
				}
					...
					if (matches[pathIdx]) {
						if (next->match) {
							...
							if (next->numOfPatterns == 0)
							{
								...
								nexts[pathIdx].hoffmanCode[nexts[pathIdx].numMessages] = next->huffman;
								nexts[pathIdx].hoffmanCodeLen[nexts[pathIdx].numMessages++] = next->huffman_len;
							}
							else
							{
								int k;
								for(k=0;k<next->numOfPatterns;k++)
								{
									nexts[pathIdx].messages[nexts[pathIdx].numMessages] = next->patterns[k];
									nexts[pathIdx].messagesLen[nexts[pathIdx].numMessages] = strlen(next->patterns[k]);
									nexts[pathIdx].hoffmanCode[nexts[pathIdx].numMessages] = next->hoffmanCodes[k];
									nexts[pathIdx].hoffmanCodeLen[nexts[pathIdx].numMessages++] = next->hoffmanCodesLength[k];
								}
							}
						}
						j++;
				...
				if (/*next == NULL || */next->hasFailInto || next->numGotos > 1) {
					...
					if (nextPtr.match) {
						...
						for (i = 0; i < nextPtr.numMessages; i++) {
							...
							nexts[pathIdx].hoffmanCode[nexts[pathIdx].numMessages] = nextPtr.hoffmanCode[i];
							nexts[pathIdx].hoffmanCodeLen[nexts[pathIdx].numMessages++] = nextPtr.hoffmanCodeLen[i];
...
void setSimpleStatesData(StateMachine *machine, ACTree *tree, int verbose) {
	...
	while (!(nodequeue_isempty(&queue))) {
		...
		if (match) {
			...
			//patterntable_add_pattern(machine->patternTable, node->id, 0, node->message, node->messageLength, node);

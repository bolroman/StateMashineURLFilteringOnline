...
void putStates(TableStateMachine *machine, ACTree *tree, int verbose) {
	...
	while (!nodequeue_isempty(&queue)) {
		...
		node->marked = 1;
	//	printf("%d %d %d %d %s;", tree->root->id, node->id, node->match, node->numGotos, node->message); fflush (stdout);
    //  printNode(node);
#ifdef USE_DEPTHMAP
...
TableStateMachine *generateTableStateMachine(Anchor *path, int verbose) {
	ACTree tree;

...
#define GET_NUM_GOTOS_LT(state, res) 									\
{																		\
	res = (((StateHeader*)(state))->size << 8) | ((state)[1] & 0x0FF); 	\
}
	...
	do {																																		\
		...
		if (table[s_idx] != (uchar)(-1) || table[s_idx + 1] != (uchar)(-1)) { /* There is a goto */												\
			...
			if ((verbose) && (result)->match) {																									\
				...
				ind = FAIL_IDX + 2;																												\
				for (i = 0; i < 32; i++) {																										\
					if (((lookupTableState)[ind + i] & 1) != 0 && i * 8 < (int)c) count++;														\
					if (((lookupTableState)[ind + i] & 2) != 0 && i * 8 + 1 < (int)c) count++;													\
					if (((lookupTableState)[ind + i] & 4) != 0 && i * 8 + 2 < (int)c) count++;													\
					if (((lookupTableState)[ind + i] & 8) != 0 && i * 8 + 3 < (int)c) count++;													\
					if (((lookupTableState)[ind + i] & 16) != 0 && i * 8 + 4 < (int)c) count++;													\
					if (((lookupTableState)[ind + i] & 32) != 0 && i * 8 + 5 < (int)c) count++;													\
					if (((lookupTableState)[ind + i] & 64) != 0 && i * 8 + 6 < (int)c) count++;													\
					if (((lookupTableState)[ind + i] & 128) != 0 && i * 8 + 7 < (int)c) count++;												\
					if (i * 8 + 8 >= (int)c)																									\
						break;																													\
				}																																\

...
//	node->hoffmanCodesLength[node->numOfPatterns] = sizeOfHoffmanCode;
//
//	node->numOfPatterns++;
//}
//
...
//
//        	//RB
////        	int sizeOfHoffmanCode = (state->failure.pattern_huffman_len[i])/8 + 1;
////        	state->pattern_huffman[state->numMatches] = (unsigned char*)malloc(sizeof(unsigned char) * sizeOfHoffmanCode);
////        	memset(state->pattern_huffman[state->numMatches],0, sizeof(unsigned char) * sizeOfHoffmanCode);
////        	memcpy(state->pattern_huffman[state->numMatches], state->failure.pattern_huffman[i], sizeOfHoffmanCode);
////        	state->pattern_huffman_len[state->numMatches] = sizeOfHoffmanCode;
////
